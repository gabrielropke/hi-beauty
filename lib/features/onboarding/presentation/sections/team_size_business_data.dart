...
class TeamSizeBusinessData extends StatelessWidget {
  final OnboardingLoaded state;
  const TeamSizeBusinessData({
    super.key,
    required this.state,
  });
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  l10n.yourBusinessSizeTeam,
                  style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w600),
                ),
              ],
            ),
            SizedBox(height: 32),
            _TeamSizeWrap(teamSize: state.teamSize),
...
class _TeamSizeWrap extends StatelessWidget {
  final List<TeamSizeModel> teamSize;
  const _TeamSizeWrap({required this.teamSize});
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final itemWidth =
            (maxWidth - (horizontalSpacing * 1)) / 1;
        return Wrap(
          spacing: horizontalSpacing,
          runSpacing: verticalSpacing,
          children: teamSize.map((s) {
            final selectedKey =
...
class _Item extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      onTap: () => context.read<OnboardingBloc>().add(SelectTeamSize(keyValue)),
      child: Container(
        height: 50,
